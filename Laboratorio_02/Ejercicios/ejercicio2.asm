...
loop:   bge $s5, $a2, done # while i<FB_LENGHT
	...
	andi $t2, $s6, 0xff000000 # t2 = color Azul de la imagen
	andi $t3, $s6, 0x00ff0000 # t2 = color Azul de la imagen
	andi $s6, $s6, 0xff000000 # s6 = colores NA y Azul de la imagen
	
	srl $t2, $t2, 8 # desplazo t2 8 bits a la izquierda (al sector de verde)
	or $s6, $s6, $t2 # copio el tono de color azul al sector de verde
	
	srl $t2, $t2, 8 # desplazo t2 8 bits a la izquierda (al sector de rojo)
	or $s6, $s6, $t2 # copio el tono de color azul al sector de rojo
	
	srl $t2, $t2, 8 # desplazo t2 8 bits a la izquierda (al sector de rojo)
	or $s6, $s6, $t2 # copio el tono de color azul al sector de rojo
	or $s6, $s6, $t3 # copio el tono de color azul al sector de rojo
