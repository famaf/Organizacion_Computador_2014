...
loop:   bge $s5, $a2, done # while i<FB_LENGHT
	...
	andi $t2, $s6, 0x0000ff00 # t2 = componente Verde del pixel
	andi $s6, $s6, 0xff00ff00 # s6 = alpha y Verde del pixel
	
	sll $t2, $t2, 8 # desplazo t2 8 bits a la izquierda (al sector de rojo)
	or $s6, $s6, $t2 # copio la intensidad de color verde al sector de rojo
	
	srl $t2, $t2, 16 # desplazo t2 16 bits a la derecha (al sector de azul)
	or $s6, $s6, $t2 # copio la intensidad de color verde al sector de azul
